...
export default class Simulation {
    ...
    computeDensityPressure = () => {
        ...
        particles.forEach((p) => {
            ...
            p.neighbors.forEach((n) => {
                ...
                p.rho += MASS * POLY6 * Math.pow(HSQ - r2, 3);
            })
            p.rho += MASS * POLY6 * Math.pow(HSQ, 3.0);
    ...
    computeForces = () => {
        const { particles } = this;
        particles.forEach((p) => {
            let fpress = new Vector2(0.0, 0.0);
            let fvisc = new Vector2(0.0, 0.0);
            p.neighbors.forEach((n) => {
                let rij = n.pos.clone().sub(p.pos);
                const r = rij.length();
                let temp = -MASS * (p.p + n.p) / (2.0 * n.rho) * SPIKY_GRAD * Math.pow(H - r, 2.0);
            ...
            let fgrav = new Vector2(0.0, G * p.rho);
    ...
    integrate = () => {
        const { particles } = this;
        particles.forEach((p) => {
            let temp = p.f.clone().divideScalar(p.rho).multiplyScalar(DT);
            p.v.add(temp);
